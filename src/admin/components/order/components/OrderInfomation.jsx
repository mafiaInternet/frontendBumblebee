...
const OrderInfomation = () => {
    ...
    const [status, setStatus] = useState(order.order && order.order && order.order.orderStatus)
    const handleOrderStatus = (event) => {
        setStatus(event.target.value)
        dispatch(statusOrderHandler({orderId: order.order.id,status: event.target.value}))
    }
    useEffect(() => {
        getOrderById(order.order && order.order.id)
        console.log("abc")
    }, [status, dispatch])
  console.log(order)
        ...
        <div className='orderInformation'>
            <div className='orderInformation--container'>
                ...
                <div className='orderInformation--content--head'>
                    ...
                    <Box>
                        <span className='orderInformation--content--head--id'>{order.order.id}</span>
                        <Typography component={'span'} sx={{backgroundColor: orderStatus.find((item) => item.text === status)?.color}} className='orderInformation--content--head--status'>{status}</Typography>
                    </Box>
                </div>
                    <hr></hr>
                                ...
                                <TableBody>
                                   ...
                                   <img className='img-fluid' src={
                                    ...
                                    <p>abc</p>
                                        ...
                                        <StyledTableCell>{item.discountedPrice}</StyledTableCell>
                                        <StyledTableCell>{item.quantity}</StyledTableCell>
                                        <StyledTableCell>{item.quantity * item.discountedPrice}</StyledTableCell>
                                    </StyledTableRow>
                                    ))}
                                </TableBody>
